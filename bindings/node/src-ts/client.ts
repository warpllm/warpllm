...
export interface WarpLLMOptions {
   ...
   * own `models:`; transport only, if it names none, in which case every model
   * warpllm ships under that provider carries over and now routes to your
   * address. Either way it happens **silently** here — warpllm warns over
   * Rust's `tracing`, which this binding installs no subscriber for, so reuse
   * a built-in name deliberately. The file is read when the client is constructed, so a
