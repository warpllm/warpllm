...
/// * **Events that are not chunks.** `content_block_stop`, `message_stop` and
///   `ping` carry nothing a chunk can hold, so they ingest to `None` and this
///   loop skips them rather than emitting empty values.
...
mod tests {
    ...
    async fn keepalives_and_block_boundaries_are_skipped_not_ended() {
        ...
        // two pings, the block stop, or the message stop.
